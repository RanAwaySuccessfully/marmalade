...
func create_misc_widgets() miscWidgets {
	...
	model_input.SetText(server.Config.ModelFace)
	...
	port := strconv.Itoa(server.Config.VTSApi.Port) // convert int to string
	port_input.SetText(port)
	port_input.SetPlaceholderText("21412")

	port_input.Connect("changed", func() {
		update_numeric_config(port_input, &server.Config.VTSApi.Port)
	})
